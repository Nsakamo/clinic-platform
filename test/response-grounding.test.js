...
test("類似度や確認条件を満たさない過去対応例は料金の根拠にならない", () => {
...
test("類似するスタッフ確定例は同種の料金回答へ再利用できる", () => {
  const result = evaluateResponseGrounding({
    query: "送料はいくらですか",
    draft: "送料は500円です。",
    precedentMatches: [{ id: 7, trusted: true, score: 0.82, confirmedCount: 2 }],
    learningExampleCount: 1,
  });
  assert.equal(result.autoSendAllowed, true);
  assert.ok(result.sources.includes("類似するスタッフ確定例"));
});
