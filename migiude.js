...
// Vercel Deployment Protection を維持したまま staging のサーバー間通信だけを許可する。
// 本番では未設定のままでよい。値はRailwayの環境変数にのみ保存し、ブラウザやログへ出さない。
const PARTNER_VERCEL_BYPASS_SECRET = process.env.PARTNER_VERCEL_BYPASS_SECRET || "";
function partnerHeaders(contentType) {
  const headers = { "x-partner-key": PARTNER_KEY };
...
function isAllowedAppOrigin(req, origin) {
  const expected = requestPublicBase(req);
  // ローカル実ブラウザ確認だけは同一localhost/127.0.0.1のHTTP Originを許可する。
  // 管理環境ではrequestPublicBaseがHTTPS固定値になるため、この分岐には入らない。
  if (/^http:\/\/(localhost|127\.0\.0\.1|\[::1\])(?::\d+)?$/i.test(String(origin || "")) && origin === expected) return true;
  const normalized = normalizePublicBase(origin);
  if (!normalized) return false;
  return normalized === expected || ADDITIONAL_APP_ORIGINS.has(normalized);
}
...
const LEARNING_INTENT_LABELS = {
  booking_change: "予約変更", booking_cancel: "予約キャンセル", booking_slots: "空き枠確認", shipping_time: "発送時期",
  booking_new: "新規予約", booking_late: "遅刻連絡", shipping_fee: "送料", delivery_issue: "配送トラブル", return_exchange: "返品・交換",
  price: "料金", payment: "支払い", refund: "返金", resend: "再送", hours: "営業時間", access: "アクセス",
  menu_treatment: "施術・メニュー", eligibility: "施術対象", preparation: "来院前準備", aftercare: "施術後相談",
  campaign: "キャンペーン", tickets_points: "回数券・ポイント", documents: "書類", account: "アカウント",
  stock: "在庫", contact: "連絡先", other: "その他",
};
function learningIntentKey(q) { return Array.from(intentTokens(q || ""))[0] || "other"; }
function sanitizeLearningMeta(value) {
  const m = value && typeof value === "object" && !Array.isArray(value) ? value : {};
  return {
    intent: String(m.intent || "").trim().slice(0, 80),
    decision: String(m.decision || "").trim().slice(0, 600),
    conditions: String(m.conditions || "").trim().slice(0, 400),
    avoid: String(m.avoid || "").trim().slice(0, 400),
    searchTerms: Array.isArray(m.searchTerms) ? m.searchTerms.map(x => String(x || "").trim().slice(0, 60)).filter(Boolean).slice(0, 12) : [],
    scope: m.scope === "reusable" ? "reusable" : "one_off",
...
async function exampleLearningMetaUpdate(t, id, value) {
  ...
  ex.learningMeta = sanitizeLearningMeta(Object.assign({}, value, { updated: Date.now() }));
  if (pool) await pool.query("UPDATE examples SET learning_meta=$1 WHERE tenant=$2 AND id=$3", [ex.learningMeta, t.slug, ex.id]);
  return ex.learningMeta;
}
function learningPerformanceEntry(t, key) {
  const all = t.config.learningPerformance || (t.config.learningPerformance = {});
  const cur = all[key] && typeof all[key] === "object" ? all[key] : (all[key] = {});
  cur.total = Math.max(0, Number(cur.total || 0)); cur.unchanged = Math.max(0, Number(cur.unchanged || 0));
  cur.corrected = Math.max(0, Number(cur.corrected || 0)); cur.conflicts = Math.max(0, Number(cur.conflicts || 0));
  return cur;
}
async function ensureLearningPerformanceSeeded(t) {
  if (t.config.learningPerformanceSeededAt) return;
  for (const ex of Object.values(t.examples || {})) {
    const key = learningIntentKey(ex.q), p = learningPerformanceEntry(t, key), count = Math.max(1, Number(ex.confirmedCount || 1));
    const corrected = !!String(ex.instr || "").trim() || (!!String(ex.draft0 || "").trim() && String(ex.draft0).trim() !== String(ex.final || "").trim());
    p.total += count; if (corrected) p.corrected += count; else p.unchanged += count;
  }
  t.config.learningPerformanceSeededAt = Date.now();
  await saveTenantConfig(t);
}
async function recordLearningPerformance(t, q, changed, conflict) {
  const key = learningIntentKey(q), p = learningPerformanceEntry(t, key);
  p.total++; if (changed) p.corrected++; else p.unchanged++; if (conflict) p.conflicts++; p.updated = Date.now();
  await saveTenantConfig(t);
  return learningAutomationReadiness(t, q);
}
function learningAutomationReadiness(t, q) {
  const key = learningIntentKey(q), p = learningPerformanceEntry(t, key), total = p.total;
  const correctionRate = total ? p.corrected / total : 1;
  const pendingConflict = learningConflicts(t, true).some(item => learningIntentKey(item.q) === key);
  const ready = key !== "other" && total >= 5 && p.unchanged >= 3 && !pendingConflict && correctionRate <= 0.4;
  return { key, label: LEARNING_INTENT_LABELS[key] || key, total, unchanged: p.unchanged, corrected: p.corrected, conflicts: p.conflicts, pendingConflict, correctionRate, ready };
}
function applyLearningReadinessGate(t, q, grounding) {
  const readiness = learningAutomationReadiness(t, q), g = grounding || {};
  const sources = Array.isArray(g.sources) ? g.sources : [], domains = Array.isArray(g.domains) ? g.domains : [];
  const precedentOnly = sources.includes("類似するスタッフ確定例")
    && !sources.some(source => source === "店舗ルール" || source === "本人確認済み予約・患者情報" || source === "リアルタイム空き枠");
  const changingFact = domains.some(domain => domain === "money" || domain === "policy");
  if (precedentOnly && changingFact && !readiness.ready) {
    g.autoSendAllowed = false; g.reasons = Array.isArray(g.reasons) ? g.reasons : [];
    g.reasons.push(readiness.pendingConflict
      ? readiness.label + "の学習内容に未解決の矛盾があります"
      : readiness.label + "は承認実績が不足しています（5件以上・無修正3件以上が目安）");
    g.reasons = Array.from(new Set(g.reasons));
  }
  return readiness;
}
function learningPerformanceSummary(t) {
  return Object.keys(t.config.learningPerformance || {}).map(key => {
    const p = learningPerformanceEntry(t, key), total = p.total, correctionRate = total ? p.corrected / total : 1;
    const pendingConflict = learningConflicts(t, true).some(item => learningIntentKey(item.q) === key);
...
async function checkConflict(t, q, newFinal, excludeId) {
  ...
  try {
    ...
    if (typeof o.conflict !== "boolean") throw new Error("invalid_conflict_result");
    if (o && o.conflict === true) return { oldId: old.id, oldQ: old.originalQ || old.q, oldFinal: old.final };
  } catch (e) {
    // 判定不能を「矛盾なし」にも偽の矛盾にもせず、自動学習全体を再利用不可で終了する。
    console.error("example conflict check:", e && e.message);
    throw new Error("learning_conflict_verification_unavailable");
  }
  return null;
}
...
async function distillRules(t, c, opts) {
  try {
    ...
    const sys = "あなたはクリニック受付のナレッジ管理者。スタッフが患者に送った返信、AIの初回下書き、スタッフの修正指示、最終返信から、店舗ルールと再利用できる判断手順を構造化する。"
      ...
      + "\n判断手順memoryには、この種類の質問で何を確認し、どの順序・結論で答え、何を避けるかを記録する。今回限りならscope=one_off、他の患者にも再利用できるならscope=reusable。searchTermsには同じ意図の言い換えを入れる。"
      + "\n出力は必ず次のJSONのみ: {\"rules\":[{\"action\":\"add|update\",\"targetId\":数値またはnull,\"title\":\"短い見出し\",\"content\":\"事実を簡潔に\"}],\"memory\":{\"scope\":\"reusable|one_off\",\"intent\":\"問い合わせの種類\",\"decision\":\"回答時に取る判断・案内手順\",\"conditions\":\"適用条件\",\"avoid\":\"避ける回答\",\"searchTerms\":[\"言い換え\"]}}"
      + "\nルールは最大2件。確実な店舗事実が無ければrulesは空配列。判断手順を安全に一般化できなければmemory.scopeをone_offにする。";
    const u = "【患者の問い合わせ】\n" + (q || "（なし）")
    ...
    const memory = sanitizeLearningMeta(Object.assign({}, out && out.memory, { updated: Date.now() }));
    if (memory.scope === "reusable" && memory.intent && memory.decision && opts && opts.exampleId) {
      try { await exampleLearningMetaUpdate(t, opts.exampleId, memory); } catch (e) { console.error("learning meta:", e && e.message); }
    }
...
function learningScopeSuggestion(instr) {
  ...
  if (/(今回は|今回だけ|今だけ|一旦|とりあえず|この返信だけ)/.test(text)) return "none";
  if (/(この患者|この方|このお客様|この人|この会話)/.test(text)) return "patient";
...
function learningReviewPayload(ex, opts) {
  ...
  return {
    ...
    q: String(opts && opts.q || "").slice(0, 600),
...
async function auditReusableLearningPrivacy(t, input, meta) {
  const system = "あなたは医療機関の個人情報監査者です。学習候補が別患者の回答へ再利用可能かを厳格に判定してください。氏名、連絡先、住所、会員・保険・予約等の識別番号、具体的な予約日時、患者固有の症状・事情、今回限りの特例が候補に残る場合はsafe=falseです。一般的な画面名、業務手順、確認項目だけならsafe=trueです。迷う場合や判断材料不足はsafe=false。JSONのみ: {\"safe\":true|false,\"reason\":\"短い理由\"}";
  const payload = {
    source: {
    ...
    candidate: sanitizeLearningMeta(meta),
  };
  try {
    const raw = await aiChat(t, system, [{ role: "user", content: JSON.stringify(payload) }], 240, "critical");
    const match = String(raw || "").match(/\{[\s\S]*\}/), parsed = match ? JSON.parse(match[0]) : null;
    return !!(parsed && parsed.safe === true);
  } catch (e) {
    console.error("learning privacy audit:", String(e && e.message || e).slice(0, 80));
    return false;
  }
}
async function proposeContextualLearning(t, input) {
  ...
  try {
    ...
    if (meta.scope === "reusable") {
      const containsSpecificData = learningMetaContainsPatientSpecificData(meta, input);
      const privacyApproved = !containsSpecificData && await auditReusableLearningPrivacy(t, input, meta);
      if (!privacyApproved) meta.scope = "one_off";
    }
    return { text, meta };
  } catch (e) {
...
async function checkFormalRuleConflict(t, q, proposal, newId, source) {
  ...
  try {
    ...
    if (typeof out.conflict !== "boolean") throw new Error("invalid_conflict_result");
    const rule = related.find(item => Number(item.id) === Number(out.ruleId));
    if (out.conflict === true && !rule) throw new Error("invalid_conflict_rule");
    ...
    // 外部AI障害を偽の矛盾として大量登録せず、自動学習全体を再利用不可で終了する。
    console.error("formal rule conflict:", e && e.message);
    throw new Error("formal_rule_verification_unavailable");
  }
  return null;
}
async function proposeConflictResolution(t, item, instruction) {
  const system = "あなたは受付AIの学習ルール編集者です。現在のルールと新しい候補の食い違いについて、スタッフの指示を最優先に、両立する条件を明確にした正式ルール案を1つ作ってください。患者固有の氏名・日時・会員IDは一般化し、推測を追加しません。JSONのみ: {\"proposal\":\"確定候補の本文\"}";
  const user = "【現在】\n" + String(item.oldFinal || "").slice(0, 1800)
    + "\n\n【今回】\n" + String(item.newFinal || "").slice(0, 1800)
    + "\n\n【スタッフの指示】\n" + String(instruction || "").slice(0, 1200);
  const raw = await aiChat(t, system, [{ role: "user", content: user }], 900, "learning");
  const match = String(raw || "").match(/\{[\s\S]*\}/), parsed = match ? JSON.parse(match[0]) : {};
  return String(parsed.proposal || "").trim().slice(0, 2400);
}
...
async function processLearningJob(t, jobId) {
  ...
  try {
    ...
    // 一時的なAI・DB障害でも送信済み対応例を削除しない。再利用不可のまま監査用に保持する。
    if (job.exampleId) try { await exampleLearningMetaUpdate(t, Number(job.exampleId), { scope: "one_off" }); }
    catch (preserveError) { console.error("learning example preserve:", preserveError && preserveError.message); }
    job.status = "completed"; job.resultType = "error"; job.error = String(e && e.message || e).slice(0, 120);
    job.outcome = { type: "error", title: "自動学習を完了できませんでした", message: "今回限りの対応として安全に保存し、他の患者への回答には利用しません" };
    job.payload = null; job.updatedAt = Date.now();
  } finally {
    runningLearningJobs.delete(key);
    while (t.config.learningJobs.length > 80) {
      const removable = t.config.learningJobs.findIndex(item => item && ["done", "completed"].includes(item.status));
      if (removable < 0) break;
      t.config.learningJobs.splice(removable, 1);
    }
...
async function queueStaffLearning(t, c, opts) {
  ...
  // 患者へ実際に送信できた回答を人の確認済み結果として自動学習する。
  // AIの下書きや未送信の修正途中は保存せず、処理中の候補は安全確認が終わるまで回答生成へ使わない。
  const ex = await exampleAdd(t, { q, final, draft0, instr, learningChat, source: opts.source || "web" });
  if (!ex) return { learnedId: null, job: null };
  ...
  const now = Date.now(), job = {
    ...
    exampleId: ex.id, reused: !!ex.reused, status: "processing", resultType: "", createdAt: now, updatedAt: now, learningReview: review, conflict: null, outcome: null,
  ...
  try {
    ...
    // ジョブを永続化できない時は、再起動で失う前にこの要求内で安全判定まで完了させる。
    console.error("learning job initial save:", e && e.message);
...
async function fetchBooking(t, c) {
  try {
    ...
    let resp;
    try { resp = await fetch(url, { headers: partnerHeaders(), signal: ctrl.signal }); }
...
async function baCall(t, c, action, extra, timeoutMs) {
  try {
    ...
    let r;
    try { r = await fetch(BA_URL, { method: "POST", headers: partnerHeaders("application/json"), body: JSON.stringify(body), signal: ctrl.signal }); }
    finally { clearTimeout(timer); }
    if (!r) return null;
...
async function genDraft(t, c, opts) {
  ...
  try {
    ...
    if (out && typeof out === "object") {
      ...
      out.learningUsage = {
        rules: Array.isArray(out.grounding && out.grounding.ruleRefs) ? out.grounding.ruleRefs : [],
        prefs: (Array.isArray(S(t).prefs) ? S(t).prefs : []).map(p => String(typeof p === "string" ? p : ((p && p.text) || ""))).filter(Boolean).slice(-8),
        examples: out.learningRefs,
      };
    }
    return out;
  } catch (e) { return null; }
  ...
  for (const ev of (req.body.events || [])) {
    try {
      ...
      try {
        ...
        catch (learningError) { console.error("staff line learning queue:", learningError && learningError.message); }
        await staffLineReply(t, ev.replyToken, [staffLineText("✅ " + staff.name + "さんの承認内容を患者様へ送信しました。" + (learned && learned.learnedId ? "\n🧠 この対応を次回以降の類似問い合わせに反映します。" : ""))]);
      } catch (e) {
...
app.get("/api/learning-pending-count", guard, (req, res) => {
  ...
  res.json({
    ok: true,
    decisions: jobs.filter(job => job.status === "awaiting_decision" || (job.status === "ready" && job.resultType === "review")).length,
  ...
  try {
    ...
    if (scope === "learn") {
      ...
      if (job && ["awaiting_decision", "awaiting_consent"].includes(job.status)) {
        ...
        job.learningReview = Object.assign({}, job.learningReview || {}, { exampleId: ex.id, text });
        job.payload = { q: ex.q, final: ex.final, draft0: ex.draft0, instr: ex.instr, learningChat: ex.learningChat, reviewText: text, source: ex.source || "web", changed: ex.final !== ex.draft0 || !!ex.instr };
        job.updatedAt = Date.now();
      }
...
app.post("/api/learning-conflict-consult", guard, async (req, res) => {
  const t = req.tenant, item = learningConflicts(t, true).find(conflict => conflict.id === String(req.body.id || ""));
  if (!item) return res.status(404).json({ ok: false, error: "not_found" });
  const instruction = String(req.body.instruction || "").trim().slice(0, 1200);
  if (!instruction) return res.status(400).json({ ok: false, error: "required" });
  try {
    const proposal = await proposeConflictResolution(t, item, instruction);
...
async function partnerGet(path) {
  ...
  try {
    const r = await fetch(PARTNER_BASE + path, { headers: partnerHeaders(), signal: AbortSignal.timeout(8000) });
...
async function partnerPost(path, body) {
  if (!PARTNER_KEY) return { ok: false, status: 0, json: null };
  try {
    const r = await fetch(PARTNER_BASE + path, { method: "POST", headers: partnerHeaders("application/json"), body: JSON.stringify(body || {}), signal: AbortSignal.timeout(8000) });
    return { ok: r.ok, status: r.status, json: await r.json().catch(() => null) };
  } catch (e) { return { ok: false, status: 0, json: null }; }
}
...
async function partnerReq(method, path, body) {
  if (!PARTNER_KEY) return { ok: false, status: 0, json: null };
  try {
    const opt = { method, headers: partnerHeaders(body != null ? "application/json" : ""), signal: AbortSignal.timeout(8000) };
...
app.post("/api/draft-chat", guard, async (req, res) => {
  ...
  try {
    ...
    if (out.rule && typeof out.rule === "object") {
      const title = String(out.rule.title || "").trim().slice(0, 100), content = String(out.rule.content || "").trim().slice(0, 1000);
...
app.post("/api/draft-chat-stream", guard, async (req, res) => {
  ...
  try {
    ...
    if (full) {
      ...
      savedMem = String(mm ? mm[1].split(/@@/)[0] : "").trim().slice(0, 200);
      const rm = full.match(/@@RULE@@\s*([\s\S]*)$/);
      if (rm) {
        const rtxt = rm[1].split(/@@/)[0].trim();
        const jm = rtxt.match(/\{[\s\S]*?\}/);
        if (jm) {
          try {
            const obj = JSON.parse(jm[0]), title = String(obj.title || "").trim().slice(0, 100), content = String(obj.content || "").trim().slice(0, 1000);
            if (title && content) savedRule = { title, content };
          } catch (e) {}
        }
      }
  ...
  try {
    ...
    for (const item of due) {
      ...
      try {
        ...
        if (item.text) {
          queueStaffLearning(t, c, { q: item.question, final: item.text, draft0: item.draft0, instr: item.instr, learningChat: item.learningChat, reviewText: item.learningText, source: "web-scheduled" }).catch(e => console.error("scheduled learning:", e && e.message));
        }
      } catch (e) {
...
app.get("/login/email", (req,res)=>{
  ...
  if (!url) return res.status(503).send(pageWithEnvironmentBanner(req, '<!doctype html><html lang="ja"><meta charset="utf-8"><body><p>メール認証の接続先を確認できません。運営へお問い合わせください。</p><a href="/">ログイン画面へ戻る</a></body></html>'));
...
app.get("/", (req, res) => { res.set("Content-Type", "text/html; charset=utf-8"); res.set("Cache-Control", "no-store"); res.send(pageWithEnvironmentBanner(req, tenantFromReq(req) ? PAGE : emailLoginPage(LOGIN_PAGE, requestPublicBase(req)))); });
app.get("/signup", (req, res) => res.redirect("/")); // 申込みは営業契約後に運営が作成
app.get("/board", (req, res) => { res.set("Content-Type", "text/html; charset=utf-8"); res.set("Cache-Control", "no-store"); res.send(pageWithEnvironmentBanner(req, tenantFromReq(req) ? BOARD_PAGE : emailLoginPage(LOGIN_PAGE, requestPublicBase(req)))); });
...
<!-- email-login-start --><a href="/login/email" style="display:block;text-align:center;padding:12px;border-radius:8px;background:#06c755;color:#fff;font-size:15px;font-weight:600;text-decoration:none;">メール認証でログイン</a>
<p style="font-size:12px;color:#6b7280;line-height:1.6;">うけつけるんに登録済みのメールで認証すると、連携中の右腕くんが開きます。パスワードは不要です。</p>
<div style="font-size:12px;color:#6b7280;margin:16px 0 10px;border-top:1px solid #e5e7eb;padding-top:12px;">右腕くん単独利用・従来のログイン</div><!-- email-login-end -->
...
<div id="setPop"><div class="settingsCard">
  ...
  <div class="settingsSection">
    ...
    <div style="font-size:11px;color:#6b7280;margin-top:5px;line-height:1.55;">右腕くん画面またはスタッフLINEで確定して送った返信を学習し、似た問い合わせへ再利用します。LINE公式アカウント管理画面から直接送った返信は右腕くんへ届かないため学習できません。店舗ルール、全体の返信方針、過去の対応・学習例はここで確認・編集・削除できます。<span id="learningAutomationSummary"></span><span id="learningConflictSummary"></span></div>
  </div>
  ...
  <div class="settingsFooter"><button class="cbtn" onclick="closeSet()">キャンセル</button><button id="saveSettingsBtn" class="cbtn send" onclick="saveSet()">設定を保存</button></div>
</div></div>
...
<div id="conflictPop" style="position:fixed;inset:0;background:rgba(15,23,42,.48);z-index:92;display:none;align-items:center;justify-content:center;">
  <div class="learningConflictCard">
    <div class="learningConflictHead"><h3>🧠 学習内容の確認・右腕くん</h3><button type="button" class="cbtn" onclick="closeConflictThread()">閉じる</button></div>
    <div id="conflictThread" class="learningConflictThread">
      <div class="learningConflictLine"><div class="learningConflictAvatar">AI</div><div class="learningConflictMessage">
        <div id="conflictTitle" style="font-weight:800;margin-bottom:5px;">現在の内容と今回の学習候補に違いが見つかりました</div>
        <div id="conflictQuestion" style="color:#64748b;margin-bottom:8px;"></div>
        <div class="learningConflictDiff">
          <div class="learningConflictRule old"><b id="conflictOldLabel">現在の正式ルール</b><span id="conflictOld"></span></div>
          <div class="learningConflictRule new"><b id="conflictNewLabel">今回の学習候補</b><span id="conflictNew"></span></div>
        </div>
        どのように整理しますか？文章で教えても、下の候補を選んでも構いません。
        <div class="learningConflictQuick">
          <button id="conflictOldButton" type="button" class="cbtn" onclick="resolveConflict('old',this)">現在を残す</button>
          <button id="conflictNewButton" type="button" class="cbtn" onclick="resolveConflict('new',this)">今回へ変更</button>
          <button type="button" class="cbtn" onclick="prefillConflictInstruction('現在と今回の内容は適用条件が違うので、条件を分けて両方残して')">条件で使い分け</button>
          <button type="button" class="cbtn" onclick="resolveConflict('old',this)">今回の候補を削除</button>
        </div>
      </div></div>
      <div id="conflictUserLine" class="learningConflictUser"><div id="conflictUserText"></div></div>
      <div id="conflictProposalLine" class="learningConflictLine learningConflictProposal"><div class="learningConflictAvatar">AI</div><div class="learningConflictMessage">次の内容なら矛盾せず保存できます。<div id="conflictProposalText" class="learningConflictRule new" style="margin-top:8px;"></div><div style="display:flex;justify-content:flex-end;gap:7px;margin-top:8px;"><button type="button" class="cbtn" onclick="focusConflictInstruction()">修正を伝える</button><button id="conflictConfirmCustom" type="button" class="cbtn send" onclick="confirmConflictProposal(this)">この内容で確定</button></div></div></div>
    </div>
    <div class="learningConflictCompose"><textarea id="conflictInstruction" placeholder="例：通常予約は前日まで無料で、外部媒体予約だけ2日前からにして"></textarea><div class="learningConflictComposeActions"><button id="conflictConsultButton" type="button" class="cbtn send" onclick="consultLearningConflict(this)">右腕くんに送る</button></div></div>
  </div>
</div>
<script>
let DATA=[];let DATA_REV=0;let DATA_READY=false;let current=null;
const learningJobsByConversation={},shownLearningDecisions=new Set(),shownLearningResults=new Set();let learningJobList=[];
...
async function openLearningPending(){if(learningPendingCounts.conflicts>0){try{const r=await fetch("/api/learning-data"),j=await r.json();const conflict=j&&j.conflicts&&j.conflicts[0];if(conflict){showConflict(conflict);return;}}catch(e){}}if(learningPendingCounts.decisions>0){await pollLearningJobs();const job=learningJobList.find(item=>item&&item.learningReview&&(item.status==="awaiting_decision"||(item.status==="ready"&&item.resultType==="review")));if(job){if(current!==job.conversationId)openChat(job.conversationId);shownLearningDecisions.add(job.id);showLearningScope(Object.assign({},job.learningReview,{learningJobId:job.id}));return;}}openLearning();}
...
function renderList(){
  ...
  rows.forEach(r=>{ const group=r.status==="done"?"done":"todo";
    ...
    const learningJob=learningJobsByConversation[r.id],learningBadge=learningJob?('<span style="font-size:10px;color:'+(learningJob.status==="ready"?'#b91c1c':learningJob.status==="awaiting_decision"?'#92400e':'#64748b')+';white-space:nowrap;">'+(learningJob.status==="ready"?'⚠ 学習要確認':learningJob.status==="awaiting_decision"?'🧠 学習判断待ち':learningJob.status==="processing"?'… 学習確認中':'')+'</span>'):'';
...
function openRuleLearning(){
  const data=learningScopeData;if(!data)return;
  const text=(document.getElementById("learningScopeText").value||"").trim();
  const ctx={customer:data.q||"",draft0:data.draft0||"",finalText:data.final||"",instructions:text||data.instr||"",confirmedGeneral:true};
  closeLearningScope();openAsst(ctx);
}
function showLearnResult(message){const b=document.getElementById("learnToast");if(!b)return;b.textContent="✓ "+message;b.style.display="block";clearTimeout(learnToastTimer);learnToastTimer=setTimeout(()=>{b.style.display="none";},3000);}
let learningResultTimer=null;
function showLearningOutcome(outcome,jobId){const o=outcome||{},type=String(o.type||"new"),b=document.getElementById("learningResultBanner"),icons={processing:"⋯",new:"✓",updated:"✓",duplicate:"⇄",conflict:"⚠",ignored:"−",error:"!"},labels={processing:"確認中",new:"新規保存",updated:"更新完了",duplicate:"重複統合",conflict:"要確認",ignored:"学習なし",error:"エラー"};if(!b)return;clearTimeout(learningResultTimer);b.className="show "+type;document.getElementById("learningResultIcon").textContent=icons[type]||"✓";document.getElementById("learningResultTitle").textContent=o.title||"学習処理が完了しました";document.getElementById("learningResultMessage").textContent=o.message||"";document.getElementById("learningResultLabel").textContent=labels[type]||"完了";b.onclick=type==="conflict"?()=>openLearningPending():null;learningResultTimer=setTimeout(closeLearningResult,3000);if(jobId)api("/api/learning-job-ack",{id:jobId}).catch(()=>{});}
function closeLearningResult(){const b=document.getElementById("learningResultBanner");clearTimeout(learningResultTimer);learningResultTimer=null;if(b){b.className="";b.onclick=null;}}
...
function openDraftChat(){if(!current)return;const openId=current;const c=DATA.find(x=>x.id===openId);if(!c)return;
  ...
  if(s&&s.ts===c.ts){ // 会話に動きがなければ前回の続きから再開
    ...
    dHist=[];dLog=[];dSessions[current]={hist:dHist,log:dLog,ts:c.ts,memory:"",rule:null};
...
let conflictData=null,conflictProposal="";
function showConflict(c){conflictData=c;conflictProposal="";const rule=c.kind==="rule";document.getElementById("conflictTitle").textContent=rule?"正式ルールと新しい学習候補に違いが見つかりました":"似た問い合わせで、前回と今回の回答が食い違っています";document.getElementById("conflictQuestion").textContent=c.q?("対象の問い合わせ："+c.q):"";document.getElementById("conflictOldLabel").textContent=rule?"現在の正式ルール":"以前の確認済み回答";document.getElementById("conflictNewLabel").textContent="今回の学習候補";document.getElementById("conflictOld").textContent=c.oldFinal||"";document.getElementById("conflictNew").textContent=c.newFinal||"";document.getElementById("conflictOldButton").textContent=rule?"現在のルールを残す":"以前の回答を残す";document.getElementById("conflictNewButton").textContent=rule?"今回の内容へ更新":"今回の回答を正解にする";document.getElementById("conflictInstruction").value="";document.getElementById("conflictUserLine").classList.remove("show");document.getElementById("conflictProposalLine").classList.remove("show");document.getElementById("conflictPop").style.display="flex";}
function closeConflictThread(){document.getElementById("conflictPop").style.display="none";conflictData=null;conflictProposal="";}
function prefillConflictInstruction(text){const input=document.getElementById("conflictInstruction");input.value=text||"";input.focus();}
function focusConflictInstruction(){document.getElementById("conflictInstruction").focus();}
async function consultLearningConflict(btn){const c=conflictData,input=document.getElementById("conflictInstruction"),instruction=(input.value||"").trim();if(!c||!instruction){input.focus();return;}await withBusy("consult-conflict-"+c.id,btn,"確認中…",async()=>{try{document.getElementById("conflictUserText").textContent=instruction;document.getElementById("conflictUserLine").classList.add("show");const r=await api("/api/learning-conflict-consult",{id:c.id,instruction}),j=await r.json();if(!r.ok||!j.ok)throw new Error(j.error||"ai");conflictProposal=j.proposal||"";document.getElementById("conflictProposalText").textContent=conflictProposal;document.getElementById("conflictProposalLine").classList.add("show");input.value="";const thread=document.getElementById("conflictThread");thread.scrollTop=thread.scrollHeight;}catch(e){uiAlert("右腕くんが整理できませんでした。もう一度お試しください");}});}
function confirmConflictProposal(btn){if(!conflictProposal)return;resolveConflict("custom",btn,conflictProposal);}
async function resolveConflict(mode,btn,final){const c=conflictData;if(!c)return;await withBusy("resolve-conflict-"+c.id,btn,"更新中…",async()=>{try{const r=await api("/api/learning-conflict-resolve",{id:c.id,learningJobId:c.learningJobId,mode,final:final||""}),j=await r.json().catch(()=>({}));if(!r.ok||!j.ok)throw new Error("save");closeConflictThread();showLearningOutcome({type:mode==="old"?"ignored":"updated",title:mode==="old"?"現在の内容を残しました":"正式ルールを更新しました",message:mode==="old"?"今回の学習候補は回答へ反映しません":"確定した内容を今後の回答へ反映します"});pollLearningJobs();refreshLearningBadge();}catch(e){uiAlert("学習内容を更新できませんでした");}});}
// ---- settings popup ----
