...
- 設定画面に「学習データ管理」を追加。店舗ルール、スタッフの記憶、過去の対応例は用途と優先度が違うため
  保存先を統合せず、同じ管理画面内の3タブとして一覧・検索・編集・削除できる。店舗ルールとスタッフの
  記憶は手動追加も可能。過去の対応例は実際に送信した返信からだけ自動生成し、元のAI下書きは監査用に
  読み取り専用で表示する。
- 設定画面と学習データ管理をレスポンシブ化。PCは設定項目を2列のカードで整理して横幅を活用し、スマホは
  全画面1列、固定ヘッダー・フッター、44px前後の操作領域、3タブ＋全幅検索欄に切り替える。
...
- `git diff --check`: 成功。
- 設定画面レスポンシブ変更後の `node --check migiude.js`、埋め込み `PAGE` 構文解析、`git diff --check`: 成功。
- 学習データ管理APIの店舗ルール・スタッフの記憶について、ローカルのテスト法人で追加・一覧・編集・削除: 成功。
...
17. 設定→学習データ管理で3タブの件数と説明が表示され、店舗ルール・スタッフの記憶を追加、編集、削除できる。
18. テスト返信を送信して学習させた後、過去の対応例で問い合わせ・実返信・修正指示を編集でき、削除後は一覧から消える。
