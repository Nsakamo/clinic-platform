...
const INTENT_PATTERNS = [
  ...
  ["booking_new", /(?:予約したい|予約を取りたい|予約方法|初回予約|新規予約)/],
  ["booking_late", /(?:遅刻|遅れ(?:ます|そう)|間に合わない|何分遅れ)/],
  ["shipping_time", /(?:発送|出荷|届|到着|配送.*いつ|納期)/],
  ["shipping_fee", /(?:送料|配送料|配送.*料金)/],
  ["delivery_issue", /(?:商品.*届か|配送.*事故|荷物.*紛失|破損|誤配送)/],
  ["return_exchange", /(?:返品|交換|返品方法|交換方法)/],
  ...
  ["menu_treatment", /(?:メニュー|施術内容|コース|プラン|どの施術)/],
  ["eligibility", /(?:受けられ|対象|適応|妊娠|授乳|年齢制限|未成年)/],
  ["preparation", /(?:持ち物|準備|服装|化粧|メイク|食事.*前|来院前)/],
  ["aftercare", /(?:施術後|術後|痛み|腫れ|出血|副作用)/],
  ["campaign", /(?:キャンペーン|クーポン|割引|特典)/],
  ["tickets_points", /(?:回数券|チケット|ポイント|残高)/],
  ["documents", /(?:書類|領収書|診断書|明細|証明書)/],
  ["account", /(?:ログイン|パスワード|アカウント|登録情報|退会)/],
  ["stock", /(?:在庫|売り切れ|入荷|再入荷)/],
  ["contact", /(?:電話番号|問い合わせ先|連絡先|メールアドレス)/],
];
