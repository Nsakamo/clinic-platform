...
const tentativeApplication = /(?:チケット|充て|当て|あて|充当|適用|ご利用|お使い|利用|使)[^。]{0,25}(?:可否|(?:られる|できる|可能|いただける|ことができる|える|される|なれる|となる)か(?:どうか)?|確認のうえ)[^。]{0,30}(?:確認|確かめ|調べ|照会|検討|連絡)/;
const decidedApplication = /(?:充て|当て|あて|充当|適用)(?:(?!確認|可否|か)[^。、]){0,14}(?:いたします|します|ます|させていただきます)/;
...
function explicitEditMismatch(instruction, draft, previousDraft = "") {
  ...
  const explicitReversal = checksApplication
    || /(?:チケット|キャンセルに充て|充て|当て|あて|充当|適用|使える|利用|確定)[^。]{0,25}(?:確認|確かめ|照会|調べ|聞|保留|やめ|戻|可否)|(?:確認|確かめ|照会|調べ|聞)[^。]{0,25}(?:チケット|キャンセルに充て|充て|当て|あて|充当|適用|使える|利用)|(?:まだ決まっていない|まだ決まってない|未確定|未決定).{0,15}(?:確認|保留|待ち)|(?:最初|元).{0,8}(?:文|下書き).{0,8}戻/.test(nonPatientAsk);
