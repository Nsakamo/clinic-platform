...
const applicationMention = /(?:充て|当て|あて|充当|適用|(?:ご利用|お使い|利用|使).{0,8}(?:でき|可能|える|いただけ|なれ))/;
...
function explicitEditMismatch(instruction, draft, previousDraft = "") {
  ...
  const decisionRequested = noConfirmation || !/(?:確認|確かめ|照会)/.test(remaining)
    || (applying && /ご確認/.test(remaining) && !checksApplication);
  if (noConfirmation
  ...
  if (applying && decisionRequested && !confirmationRequested
      && tentativeApplication(answer) && !decidedApplication.test(answer)) return "適用する指示が、適用できるか確認する案内に戻っています";
  const prior = String(previousDraft || "").replace(/\s+/g, "");
  const priorDecision = decidedApplication.test(prior) && !tentativeApplication(prior);
  const nonPatientAsk = ask.replace(/ご確認|確認してもら(?:って|う)|確認いただ(?:き|いて|く)|確認してください/g, "");
  const explicitReversal = checksApplication
    || /(?:充て|当て|あて|充当|適用|使える)[^。]{0,20}(?:確認|確かめ|照会|調べ|聞|保留|やめ|見送|しない|取り消|可否)|(?:確認|確かめ|照会|調べ|聞)[^。]{0,20}(?:充て|当て|あて|充当|適用|使える)|(?:まだ決まっていない|まだ決まってない|未確定|未決定).{0,15}(?:確認|保留|待ち)|(?:最初|元).{0,8}(?:文|下書き).{0,8}戻/.test(nonPatientAsk);
  if (priorDecision && !explicitReversal
      && tentativeApplication(answer) && !decidedApplication.test(answer)) return "前の下書きで確定した適用が、最新指示なしに可否確認へ戻っています";
  return "";
}
