...
function explicitEditMismatch(instruction, draft) {
  ...
  const confirmationRequested = !noConfirmation && /(?:要確認|か(?:どうか)?確認|確認(?:中|待ち|後|予定|が必要|でき次第|が取れ次第|取れ次第|お願い|をお願い|して|する|します|いたします|しないと(?:いけない|ならない)|しなければ|が済むまで|したうえで)|確かめ(?:て|る)|照会(?:して|中|する))/.test(remaining);
  ...
  if (/(?:充て(?:る|て)|当て(?:る|て)|あて(?:る|て)|充当(?:する|して)|適用(?:する|して)|キャンセルに使(?:う|って))/.test(ask)
      && decisionRequested && !confirmationRequested
