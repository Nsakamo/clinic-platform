...
function explicitEditMismatch(instruction, draft, previousDraft = "") {
  ...
  const checksApplication = /(?:充て(?:る|られる)|当て(?:る|られる)|あて(?:る|られる)|充当(?:する|できる)|適用(?:する|できる)|使(?:う|える))か(?:どうか)?.{0,25}(?:要確認|確認|確かめ|照会)/.test(remaining);
  ...
  const priorDecision = /(?:充て|当て|あて|充当|適用)(?:ます|いたします|します|させていただきます)/.test(prior)
    && !/(?:充てられるか|充当できるか|適用できるか|使えるか).{0,30}確認/.test(prior);
  const explicitReversal = checksApplication || /(?:適用可否|充当可否|チケット.{0,12}可否|確認に戻(?:す|して)|改めて.{0,12}(?:確認|照会))/.test(ask);
  if (priorDecision && !explicitReversal
      && /(?:充てられるか|充当できるか|適用できるか|使えるか).{0,30}確認/.test(answer)) return "前の下書きで確定した適用が、最新指示なしに可否確認へ戻っています";
  return "";
}
