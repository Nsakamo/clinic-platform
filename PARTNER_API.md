# 右腕くん ⇄ うけつけるん パートナーAPI

**契約の正本（唯一の正）はうけつけるん側リポジトリの `docs/19-partner-api-contract.md`。**
API を変える／足すときは、まず正本を更新し、右腕くん（`migiude.js`）とうけつけるん（`lib/migiude.ts` / API routes）を
同じセッションで揃えること。破壊的変更は避け、フィールド追加で対応する。

## 共通メール認証入口（2026-09-08・公開準備中）

公開 `/login/email` はうけつけるんの既存メールOTPへ転送し、認証後に従来のSSOで自法人へ戻る。
本番とstagingは固定の別URLへ接続し、ブラウザのHost・任意転送URL・メール・slugを信用しない。
パートナーAPI仕様・既存セッション権限は変更せず、アカウントを自動作成しない。単独利用向けの従来ログインは残す。
うけつけるんのログイン戻り先対応を先に公開してから右腕くん側を公開する。

## この repo（右腕くん）が提供するエンドポイント（`migiude.js` 内、`pGuard` 認証）

- `GET  /api/partner/tenants` … 顧客一覧
- `POST /api/partner/tenants` … 新規作成（空テナント）`{name, accountEmail, slug?}` → `{slug,name,loginId,accountEmailConfigured}`
- `DELETE /api/partner/tenants/:slug` … 完全削除
- `GET  /api/partner/conn?slug=` … 連携状態
- `PUT  /api/partner/line-config` … LINE設定（うけつけるん運営画面から）
- `PUT  /api/partner/mail-config` … メール設定（うけつけるん運営画面から）
- `POST /api/partner/suspend` … 停止/再開
- `POST /api/partner/sso` … SSO URL発行
- `POST /api/partner/reset-login` … ログイン再発行
- `POST /api/partner/password-reset` … 登録済み管理者メールへ1時間・1回限りの再設定リンクを送信（パスワード非表示）
- `PUT  /api/partner/account` … パスワード再設定用のアカウントメールを登録・更新
- `POST /api/partner/send-line` … 患者へLINE送信
- `POST /api/partner/outbound-events` … うけつけるんがLINE送信に成功した本文・送信時刻・補助コンテンツを右腕くん履歴へ冪等同期

認証キー：`x-partner-key` == `ADMIN_SECRET`（= うけつけるん側の共有キー / `PLATFORM_SECRET`）。

## この repo が呼ぶうけつけるんのエンドポイント（`PARTNER_HOOK_URL` / `PARTNER_BOOKING_URL`）

- `POST /api/hooks/migiude` … 受信イベント転送＋AI使用量（`usage`）
  - 受信イベント転送には `slug + convId + ts` から作る `x-idempotency-key` を付ける。
  - タイムアウト、通信失敗、408/425/429/5xx は同じキーのまま最大3回再送する。
- `GET  /api/partner/booking?slug=&channel=&userId=` … AI下書き用の患者コンテキスト。
  返却 `text`（整形済み）を `bookingToText` が最優先で下書きに差し込む。
- `POST /api/partner/appointment-actions` … 予約自動受付（context/slots/propose/confirm）。
  テナント設定 `settings.bookingActions`（既定OFF）が有効なときのみ呼ぶ。本人確認と
  患者の「はい」承認（二段階）はうけつけるん側が強制する。詳細は正本 docs/19 の B-2。
- `POST /api/partner/appointment-cancel` … 顧客情報パネルからのスタッフ取消。画面で最終確認後、
  `channel + userId (+ mail時phone) + confirmed:true` を渡し、うけつけるん側で会話相手と予約患者を再照合する。

詳細な各リクエスト/レスポンス形は正本（うけつけるん `docs/19`）を参照。

stagingのうけつけるんがVercel Deployment Protection下にある場合は、Railway stagingだけに
`PARTNER_VERCEL_BYPASS_SECRET` を設定する。すべての受信転送・予約照会・患者検索・連携リクエストへ
`x-vercel-protection-bypass` としてサーバー側で付与される。保護自体は解除せず、値を画面やログへ出さない。

## 通知とスタッフ操作の責務

- 要対応・緊急問い合わせのSlack通知は右腕くんが直接行い、うけつけるん相談APIを経由しない。
- Slack Incoming Webhookは各テナントが右腕くんの設定画面で登録する。値は `config.conn.slackWebhook` に保存し、
  `CRED_KEY` 設定時は他の資格情報と同じくAES-256-GCMで暗号化する。APIや画面へ値そのものを返さない。
- `PUBLIC_BASE_URL`（または `APP_URL` / Railway公開ドメイン）が設定されていれば、Slack通知から該当会話を直接開ける。
- `settings.bookingActions=true` の連携テナントでは、スタッフ用AIチャットから予約一覧・空き枠を照会できる。
  キャンセル・日時変更は `propose` → 対象患者/予約/内容の確認カード → スタッフ承認 → `confirm` の順で行う。
- 右腕くん単体テナントでは予約操作を表示・実行せず、LINE/メール・AI・Slackだけで正常に動く。
